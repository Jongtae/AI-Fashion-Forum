...
""",
    ...
    "core": {
        ...
        "default_topic": "mock으로 구현된 패션 포럼 AI 시뮬레이션 서비스를 실제 동작하는 플랫폼으로 만들기 위해 먼저 고정해야 할 코어 개발 계획은 무엇인가?",
        "workforce_description": "AI Fashion Forum 코어 플랫폼 개발 토론 팀: CTO, Backend Engineer, Product Manager, Core Critic이 협력하여 mock 서비스를 실제 동작하는 플랫폼으로 발전시키는 핵심 개발 계획을 도출합니다.",
        ...
        "build_task_prompt": lambda topic: f"""AI Fashion Forum 프로젝트의 코어 플랫폼 개발 계획을 토론하세요.
...
- agent-core에는 content-pipeline, memory-stack, identity-update-rules, forum-generation 등의 로직이 이미 있다
      - 목표: AI 에이전트가 실제로 포럼에서 포스트를 작성하고 상호작용하는 동작하는 플랫폼을 만든다
...
## 기대 산출물
      - Core Goal (무엇이 먼저 실제로 작동해야 하는가)
      - 기술 스택 선택과 근거
      - 첫 번째 구현 범위 (First Working Loop)
...
""",
        "coordinator_prompt": COMMON_COORDINATOR_PROMPT + "\n추가 규칙: 이번 토론은 코어 플랫폼 개발 계획에 집중한다. 추상적인 설계보다 실제 구현 가능한 첫 번째 동작 루프를 우선한다.\n",
        "final_prompt": COMMON_FINAL_SYNTHESIZER_PROMPT + "\n추가 규칙: 최종 결과는 기술 스택 선택, 첫 번째 구현 범위, 개발 우선순위를 중심으로 코어 플랫폼 계획을 정리하라.\n",
        "round_focus_guide": {
            1: "코어 플랫폼의 정확한 범위와 기술 스택 선택 기준을 정의하라.",
            2: "첫 번째 구현할 코드 범위와 에이전트 동작 루프를 구체화하라.",
            3: "리스크, 기술 부채, 실패 시나리오를 점검하고 대응 방안을 제시하라.",
            4: "개발 우선순위와 스프린트 계획을 압축하라.",
            5: "이슈로 옮길 수 있게 코어 플랫폼 실행 계획을 정리하라.",
        },
        "synthesis_prompt": """위 결과를 바탕으로:
1. 코어 플랫폼의 정확한 범위와 성공 기준을 추출하고
2. 기술 스택 선택과 첫 번째 구현 범위를 정리하고
3. 개발 우선순위와 스프린트 계획을 제시하고
4. 바로 GitHub Issue로 등록 가능한 실행 초안을 작성하라.
